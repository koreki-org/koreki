...
1. **Vorbereitung**: Stelle sicher, dass die Test-Assets vorhanden sind. Führe dazu aus:
   ```bash
   powershell -File ./tests/setup-fixtures.ps1
   ```

2. **Test-Ausführung**: Führe den Golden-Thread Test via Playwright aus:
   ```bash
   npm run test:e2e tests/e2e/golden-thread.spec.ts -- --project=chromium --reporter=list
   ```
