...
5. **Agent Compliance Gate (MANDATORY):** Bevor eine Aufgabe an den User zurückgegeben wird, MUSS folgende Checkliste abgehakt werden:
   ...
   - [x] Zeigt jede `bg-*`-Klasse auf eine Farbe, die es gibt — und grenzt ein Element mit höchstens drei Mitteln ab? — *erzwungen durch [tests/unit/surface-governance.test.ts](tests/unit/surface-governance.test.ts). Eine Klasse ohne Eintrag in `tailwind.config.js` tut in Tailwind nichts: kein Fehler, keine Warnung, kein Build-Abbruch. Genau so war `bg-card` an 16 Stellen wirkungslos und Seite wie Karte rendersten dieselbe Farbe (07.09.2026). Weil dann weder Fläche noch Rand etwas trennten, wurde gestapelt statt getrennt — Rand UND Tönung UND Schatten UND Ring an einem Element. Hartcodierte Palettenfarben sind per Ratsche eingefroren (Stand 08.09.2026: 3 Dateien), ebenso die Vier-Mittel-Altfälle (1 Datei).*
