...
export const vaultService = {
    ...
    async saveSecret(key: string, value: string): Promise<void> {
        logger.debug(`[VaultService] Saving secret for ${key}... (Desktop: ${isDesktopTarget()})`);

        if (isDesktopTarget()) {
            try {
                const { invoke } = await import('@tauri-apps/api/core');
                await invoke('save_secret', { key, value });
                logger.debug(`[VaultService] Successfully saved ${key} to OS Vault.`);
            } catch (e) {
                logger.error("[VaultService] Save Error", e);
                throw new Error("Tresor konnte nicht angesprochen werden.");
            }
        } else {
            // PURE PRIVACY: RAM-only storage. Key is lost on page refresh.
            ramVault[key] = value;
            logger.debug(`[VaultService] Secret ${key} stored in RAM (Volatile).`);
        }
    },
    ...
    async getSecret(key: string): Promise<string> {
        if (isDesktopTarget()) {
            try {
                ...
                logger.error("Vault Get Error", e);
                return "";
            }
        } else {
            // Retrieve from RAM
    ...
    async deleteSecret(key: string): Promise<void> {
        logger.debug(`[VaultService] Deleting secret for ${key}...`);
        if (isDesktopTarget()) {
            try {
                const { invoke } = await import('@tauri-apps/api/core');
                await invoke('delete_secret', { key });
                logger.debug(`[VaultService] Successfully deleted ${key} from OS Vault.`);
            } catch (e) {
                logger.error("[VaultService] Delete Error", e);
            }
        } else {
            delete ramVault[key];
        }
