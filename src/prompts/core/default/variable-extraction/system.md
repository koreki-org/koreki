...
10. **STRIKTE REGEL FÜR PHYSIKALISCHE & BINÄRE MINT-GRÖSSEN (PRÄFIX-SKALIERUNG):**
    ...
    - **Konsistenz-Schutz bei fehlerhaften Schüler-Einheiten:** Schüler schreiben manchmal physikalisch falsche Einheiten auf (z. B. `I = 12 V / 6500 Ω = 0,001846 mA`). Das mathematische Ergebnis von `12/6500` ist `0.001846` (in Ampere). Der Schüler hat hier fälschlicherweise `mA` statt `A` geschrieben. Extrahiere in solchen Fällen den echten, mathematisch konsistenten Wert in der physikalischen Basiseinheit (hier: `0.001846`), damit die nachgelagerte mathematische Engine fehlerfrei rechnen kann. Das gilt analog für Datenmengen (z. B. wenn ein Schüler versehentlich `MiB` statt `GiB` schreibt, aber mathematisch korrekt gerechnet hat).
