...
Kriterien-Regeln:
1. "source" definiert, wer das Kriterium bewertet. Es gibt nur diese drei Werte, und die Wahl ist verbindlich — sie wird später nicht mehr aus der Bezeichnung abgeleitet:
   ...
   - "llm": Für alles Übrige — Ermessensfragen wie "Formel fachlich korrekt", "Ansatz nachvollziehbar", "Vorgehen sinnvoll", "Einheit begründet", UND das Einsetzen der richtigen Zahlenwerte ("Werte eingesetzt", "Einsetzen der Größen"). Ob die gegebenen Werte korrekt eingesetzt wurden, kann die Sandbox nicht belegen: Sie kennt nur den Zielwert, nicht den Rechenweg der Musterlösung.
