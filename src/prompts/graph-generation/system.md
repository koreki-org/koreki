...
6. **Sequenzielle Verkettung bei Tabellen / sequenziellen Listen (CRITICAL für VLSM/Subnetting):**
   ...
   - **WICHTIGE AUSNAHME bei nicht-sequenzieller Zuweisung (CRITICAL):**
    ...
    - Alle anderen Werte innerhalb eines Subnetzes (wie `firstHost`, `gateway`, `broadcast`) werden als **`formula`-Variablen** definiert. Achte penibel darauf, dass jede Funktion mit ihrer exakten Signatur aus dem Manifest aufgerufen wird. 
      Beispiele für korrekte Zuweisungen innerhalb desselben Subnetzes:
      - `spieler_firstHost` = `network.calculateFirstHost(spieler_netId)` (benötigt nur netId)
      - `spieler_gateway` = `network.calculateGateway(spieler_netId, spieler_mask)` (benötigt ZWEI Parameter: netId und mask!)
      - `spieler_broadcast` = `network.calculateBroadcast(spieler_netId, spieler_mask)` (benötigt ZWEI Parameter: netId und mask!)
