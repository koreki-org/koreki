                    ...
                    <div className="space-y-2">
                        ...
                        {profiles.filter(p => !p.isSystem).map(p => (
                            <div
                                ...
                                className={`w-full h-auto p-4 rounded-2xl border transition-all text-left flex justify-between items-center group cursor-pointer relative ${selectedProfileId === p.id ? 'bg-card border-primary/20 shadow-sm' : 'bg-transparent border-border hover:bg-foreground/5'}`}
                            >
