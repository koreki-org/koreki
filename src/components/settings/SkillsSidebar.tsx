...
export const SkillsSidebar: React.FC<SkillsSidebarProps> = ({
    ...
    return (
        <div 
            className={`flex-1 flex flex-col overflow-hidden relative bg-muted transition-all duration-200 h-full ${isDragging ? 'bg-primary/5 ring-2 ring-inset ring-primary' : ''}`}
            {...dragProps}
        >
            {isDragging && (
                    ...
                    <div className="space-y-2">
                        ...
                        {profiles.filter(p => !p.isSystem).map(p => (
                            <div
                                ...
                                className={`w-full h-auto p-4 rounded-2xl border transition-all text-left flex justify-between items-center group cursor-pointer ${selectedProfileId === p.id ? 'bg-card border-primary/20 shadow-sm' : 'bg-transparent border-border hover:bg-foreground/5'}`}
                            >
