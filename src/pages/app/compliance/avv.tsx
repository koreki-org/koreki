...
import { Shield, ArrowLeft, CheckCircle, Printer, Lock } from 'lucide-react';
...
interface AVVPageProps {
    content: string;
    version: string;
    isAcceptedVersion: boolean;
}

export default function AVVPage({ content, version, isAcceptedVersion }: AVVPageProps) {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground selection:bg-primary/10 selection:text-primary font-sans leading-relaxed">
            <Head>
                <title>AVV – Koreki Compliance</title>
                ...
                <div className="max-w-4xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xl font-outfit font-extrabold tracking-tighter text-foreground">
                            Koreki<span className="text-primary">.</span> Compliance
                        </span>
                    </div>
                    <Button 
                        variant="ghost" 
            ...
            <main className="max-w-4xl mx-auto px-6 py-16 md:py-24 print-container">
                <div className="relative">
                    <div className="absolute -top-12 -left-12 w-64 h-64 bg-primary/10 rounded-full blur-[120px] opacity-50 -z-10 no-print" />
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                        <h1 className="text-4xl md:text-5xl font-outfit font-extrabold text-foreground tracking-tight leading-[1.1] mb-0">
                            Rechtliche <br /> 
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary/80">Leitplanken.</span>
                        </h1>
                        
                        {isAcceptedVersion && (
                            <div className="flex items-center gap-2 bg-success/10 text-success px-4 py-2 rounded-2xl border border-success/20 no-print">
                                <CheckCircle size={16} />
                                <span className="text-xs font-bold uppercase tracking-tight">Ihre abgeschlossene Version</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-background rounded-hero p-6 md:p-card-padding shadow-xl shadow-border/50 border border-border max-w-none print-container">
                        <div className="mb-10 p-6 bg-muted rounded-2xl border border-border text-xs flex items-center gap-3 no-print">
                            ...
                            <p className="m-0 leading-relaxed font-medium">
                                Dieses Dokument ist schreibgeschützt und steht ausschließlich zur Einsicht und zum Druck gemäß den gesetzlichen Archiverungspflichten bereit.
                            </p>
                        </div>
                        ...
                        <div className="mt-20 pt-10 border-t border-border flex flex-col md:flex-row md:items-center justify-between gap-8 no-print">
                            <div className="text-xs text-muted-foreground font-bold uppercase tracking-widest">
                                Version: {version} <br />
                                Koreki Industrial Grade Security
                            </div>
                            <Button 
                                variant="outline" 
                        ...
                        <div className="hidden print:block mt-20 pt-10 border-t border-border text-[9pt] text-muted-foreground">
                            Gedruckt am {new Date().toLocaleDateString('de-DE')} | AVV Dokument | Version {version} | {isAcceptedVersion ? 'Legale Referenz' : 'Musterentwurf'}
                        </div>
                    </div>
                </div>
            </main>
